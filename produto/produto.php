<?php
    ...
    if(!empty($_GET['search']))
    {
        ...
        $sql = "SELECT p.id, p.nome, p.descricao, p.valor, p.quantidade, v.nome vendedor, c.nome categoria FROM produto p 
        JOIN vendedor v ON p.vendedor_id = v.id 
        JOIN categoria c ON p.categoria_id = c.id 
        WHERE p.nome LIKE '%$data%' or p.id LIKE '%$data%' or v.nome LIKE '%$data%' or c.nome LIKE '%$data%' 
        ORDER BY p.id DESC";
...
<script>
    ...
    function confirmDelete(id)
    {
        ...
        if(state)
        {
            window.location = 'deleteProduto.php?id='+id;
        }
    }
</script>
</html>